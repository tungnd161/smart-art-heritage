  ...
  return (
    <main className="min-h-screen bg-amber-50">
      ...
      <div className="max-w-4xl mx-auto p-6">
        <h2 className="text-xl font-bold text-red-800 mb-4">Kho Di San</h2>
        <p className="text-sm text-gray-500 mb-4">Tim thay: {diSan?.length ?? 0} di san</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {diSan?.map((ds) => (
            ...
            >
              ...
              <p className="text-right text-xs text-red-600 mt-2 font-medium">Xem chi tiet</p>
            </Link>
          ))}
        </div>
        ...
        {(!diSan || diSan.length === 0) && (
          <div className="text-center py-12 text-gray-500">
            <p>Khong tim thay di san nao</p>
            {error && <p className="text-red-500 text-sm mt-2">Loi: {error.message}</p>}
          </div>
        )}
      </div>
    </main>
