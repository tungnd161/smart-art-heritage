...
const modules = [
  { no: "1", icon: "home", title: "TRANG CHỦ", href: "/", image: "/heritages/pho-hien/01.jpg", caption: "Điểm vào dự án" },
  { no: "2", icon: "map", title: "BẢN ĐỒ DI SẢN\nHƯNG YÊN", href: "/ban-do", image: "/heritages/chua-keo/01.jpg", caption: "Khám phá Hưng Yên" },
  { no: "3", icon: "temple", title: "05 KHU DI SẢN", href: "/ban-do", image: "/heritages/dong-xam/03.jpg", caption: "Kho học liệu địa phương" },
  { no: "4", icon: "search", title: "HỒ SƠ KHÁM PHÁ\nDI SẢN", href: "/di-san/pho-hien", image: "/heritages/le-quy-don/06.jpg", caption: "Quan sát · Hotspot · 3–2–1" },
  { no: "5", icon: "photo", title: "KHÔNG GIAN\nTRỰC QUAN", href: "/di-san/pho-hien", image: "/heritages/den-tran/03.jpg", caption: "Ảnh, hotspot & góc nhìn tạo hình" },
  { no: "6", icon: "bot", title: "AI ART ASSISTANT · 5A", href: "/ai-assistant?heritage=pho-hien", image: "/heritages/pho-hien/05.jpg", caption: "AI gợi mở, học sinh quyết định" },
  { no: "7", icon: "palette", title: "XƯỞNG SÁNG TẠO", href: "/portfolio", image: "/heritages/chua-keo/06.jpg", caption: "Ý tưởng · Phác thảo · Tác phẩm" },
  { no: "8", icon: "gallery", title: "GALLERY / QR", href: "/gallery", image: "/heritages/le-quy-don/05.jpg", caption: "Trưng bày tác phẩm có phê duyệt" },
  { no: "9", icon: "chart", title: "ĐÁNH GIÁ & PHẢN TƯ", href: "/portfolio", image: "/heritages/dong-xam/06.jpg", caption: "Minh chứng quá trình học tập" },
];
...
export default function PresentationPage() {
 ...
 return <main className="system-demo">
  ...
  <section className="system-demo-body">
   ...
   <div className="system-flow top">{modules.slice(0,6).map((item,i)=><div className="system-node" key={item.no}><Node item={item} active={active===item.no} onHover={()=>setActive(item.no)}/>{i<5&&<div className="system-arrow" aria-hidden="true"><i></i><b>→</b></div>}</div>)}</div>
   <div className="turn-arrow" aria-hidden="true"><span>↓</span><i></i><span>←</span></div>
   <div className="system-flow bottom">{modules.slice(6).map((item,i)=><div className="system-node" key={item.no}><Node item={item} active={active===item.no} onHover={()=>setActive(item.no)}/>{i<2&&<div className="system-arrow" aria-hidden="true"><i></i><b>→</b></div>}</div>)}<TeacherDemo/></div>
...
function Node({item,active,onHover}:{item:typeof modules[number];active:boolean;onHover:()=>void}){return <div className={`system-card ${active?'active':''}`} onMouseEnter={onHover}><div className="node-heading"><b>{item.no}</b><span>{item.title.split('\n').map(x=><>{x}<br/></>)}</span></div><Link href={item.href} className="node-screen photo-screen" aria-label={`Mở ${item.title}`} style={{backgroundImage:`url('${item.image}')`}}><div className="screen-caption"><b className="module-icon"><Icon name={item.icon}/></b><span>{item.caption}</span><i>Mở mô-đun →</i></div></Link></div>}
